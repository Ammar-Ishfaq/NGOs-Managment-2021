...
public class Fragment6 extends Fragment implements PermissionsListener, OnMapReadyCallback {
...
//            Thread.sleep(1000);
            Mapbox.getInstance(getActivity(), Config.API);
//            Thread.sleep(1000);
        } catch (Exception e) {
            Toast.makeText(getActivity().getApplicationContext(), "Please Try Again", Toast.LENGTH_SHORT).show();
        }
        view = inflater.inflate(R.layout.select_current_location, container, false);
...
//        mapboxMap.setMaxZoomPreference(9);
    ...
    public void onMapReady(@NonNull final MapboxMap mapboxMap) {
        Fragment6.this.mapboxMap = mapboxMap;
            ...
            public void onStyleLoaded(@NonNull final Style style) {
                ...
                selectLocationButton.setOnClickListener(view -> {
                    if (hoveringMarker.getVisibility() == View.VISIBLE) {
                        ...
                        selectLocationButton.setBackgroundColor(
                                ContextCompat.getColor(getActivity().getApplicationContext(), R.color.teal_700));
    ...
    private void initDroppedMarker(@NonNull Style loadedMapStyle) {
        ...
        loadedMapStyle.addImage("dropped-icon-image", new ImageUtils().getBitmapFromVectorDrawable(getActivity().getApplicationContext(), R.drawable.blue_marker));//BitmapFactory.decodeResource(
//                this.getResources(), R.drawable.blue_marker));
    ...
    public void onExplanationNeeded(List<String> permissionsToExplain) {
        Toast.makeText(getActivity().getApplicationContext(), R.string.user_location_permission_explanation, Toast.LENGTH_LONG).show();
    }
    ...
    public void onPermissionResult(boolean granted) {
        if (granted && mapboxMap != null) {
            ...
            Toast.makeText(getActivity().getApplicationContext(), R.string.user_location_permission_not_granted, Toast.LENGTH_LONG).show();
            getActivity().finish();
        }
    }
    ...
    private void reverseGeocode(final Point point) {
        Log.d(TAG, "Longitude: " + point.latitude() + "Latitude: " + point.longitude());

        model.setLongLat(point.longitude() + "," + point.latitude());
                        ...
                        if (results.size() > 0) {
                            ...
                            mapboxMap.getStyle(new Style.OnStyleLoaded() {
                                ...
                                public void onStyleLoaded(@NonNull Style style) {
                                    if (style.getLayer(DROPPED_MARKER_LAYER_ID) != null) {
                                        Log.d(TAG, "string:" + feature.placeName());
//                                        Toast.makeText(getActivity().getApplicationContext(), "features or name:" + feature.placeName(), Toast.LENGTH_SHORT).show();
//                                        Toast.makeText(SelectLocationPickerActivity.this,
...
//                                    getString(R.string.location_picker_dropped_marker_snippet_no_results), Toast.LENGTH_SHORT).show();
    ...
    //for next BUtton Option Enabling Atrtribute
    public boolean CheckValidation() {
        Log.d(TAG, "longiandla: " + model.getLongLat());
        if (model.getLongLat().equals("")) {
            return false;
        }
        return true;
    }
